...
function showDetail() {
  ...
  if (thisProduct.discount > 0) {
    ...
    priceHTML += `<span style="text-decoration: line-through;font-size: 20px;padding-right: 12px;">${
      thisProduct.price
    }</span>
        <span style="font-size: 20px;color: #1FB3C7;">${formatCurrency(
          discountedPrice
        )}</span>`;
  } else {
  ...
  products
    .filter(
      (value) => value.id !== productId && value.type === thisProduct.type
    )
    .forEach((product) => {
      const baseURL =
      ...
      if (product.discount > 0) {
        ...
        newProduct.innerHTML = `<img src="${product.image}" alt="">
                ...
                <h2>${product.name}</h2>
                <div class="price">${formatCurrency(product.price)}VNĐ</div>`;
      }
...
function updateShoppingCart() {
  ...
  shoppingCart.forEach((item) => {
    ...
    cartItem.innerHTML = `
        ...
        <div class="content">
            <h3>${item.name}</h3>
            <span class="price">${formatCurrency(item.price)}VNĐ -</span>
  ...
  if (cartItemCountElement) {
    const cartItemCount = shoppingCart.reduce(
